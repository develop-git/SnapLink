...
public extension SnapViewsDSL {
    ...
    func horzLayout(space: Float = .offset,
                    flex: Bool = false,
                    insets: SnapViewDSL.EdgeInsets = .init(len: .offset)) -> Self
    {
        array.enumerated { v, emRef in
            let offset = emRef.isfirst ? insets.left : space
            v.lyt.vert(insets.top.extval, insets.bottom.extval)
            v.lyt.leading(by: emRef.refer?.lyt.trailing, offset: offset.extval)
            if emRef.islast {
                    ...
                    insets: SnapViewDSL.EdgeInsets = .init(len: .offset)) -> Self
    {
        array.enumerated { v, emrt in
            ...
            v.lyt.horz(insets.left.extval, insets.right.extval)
            v.lyt.top(by: emrt.refer?.lyt.bottom, offset: offset.extval)
            if emrt.islast {
    ...
    func gridLayout(colums: Int = 3,
                    layout: GridItemLayout = .equal,
                    spaces: GridItemsSpaces = .one,
                    insets: SnapViewDSL.EdgeInsets = .init(len: 10)) -> Self
    {
        array.enumeratedGrid(colums: colums) { v, prev, row, col in
            // rows
            let rowOffset = row.isfirst ? insets.top.extval : spaces.raw.row
            v.lyt.top(by: row.refer?.lyt.bottom, offset: rowOffset)
            v.lyt.bottom(.max(insets.bottom))
            
            // cols
            let colOffset = col.isfirst ? insets.left.extval : spaces.raw.col
            v.lyt.leading(by: col.refer?.lyt.trailing, offset: colOffset)
            if col.islast {
                v.lyt.trailing(insets.right.extval)
            } else {
                v.lyt.trailing(.max(insets.right))
            }
