...
internal final class SnapConstraint {
    ...
    /// - Returns: 更新布局信息
    func update(at attr: Attribute, anchor: ConstraintAnchor, extra: ExtraValue, isSuper: Bool) -> (restrict: Constraints, anchor: ConstraintAnchor)? {
...
private extension SnapConstraint {
    ...
    ///   - isSuper: 是否相对父视图进行布局
    /// - Returns: 验证结果
    func horzVerify(at index: Constraints.Attribute, isSuper: Bool) -> Bool {
    ...
    ///   - isSuper: 是否相对父视图进行布局
    /// - Returns: 验证结果
    func vertVerify(at index: Constraints.Attribute, isSuper: Bool) -> Bool {
